@extends('layouts.app')
@section('title','Galeri Foto Reimburse')
@section('page-title','Galeri Foto Lampiran Reimburse')
...
<div class="card border-0 shadow-sm mb-3">
    <div class="card-body py-3">
        <form method="GET" action="{{ route('laporan.reimburse.foto') }}" class="row g-2 align-items-end">
            <div class="col-md-3">
            ...
            <div class="col-md-3 d-flex gap-2">
                ...
                <a href="{{ route('laporan.reimburse.foto') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="col-6 col-sm-4 col-md-3 col-xl-2">
        <div class="card border-0 shadow-sm h-100" style="border-radius:10px;overflow:hidden;">
            ...
            <div class="card-body p-2">
                <div class="text-truncate fw-semibold" style="font-size:0.72rem;" title="{{ $r?->nama_pemohon }}">
                    {{ $r?->nama_pemohon ?? '-' }}
                </div>
                <div class="text-muted" style="font-size:0.68rem;">
                    <span class="badge bg-secondary me-1" style="font-size:0.6rem;">
                        {{ $r?->kategori }}
                    </span>
                    {{ $r?->cabang?->kode_cabang }}
                </div>
                ...
                <div class="d-flex gap-1 mt-1">
                    ...
                    <a href="{{ route('reimburse.lampiran.download', $foto) }}"
                       class="btn btn-outline-secondary btn-sm flex-fill py-0" style="font-size:0.65rem;">
                        <i class="bi bi-download"></i>
                    </a>
                </div>
                <div class="mt-1">
                    <a href="{{ route('reimburse.show', $r) }}" class="text-muted text-decoration-none"
                       style="font-size:0.62rem;">
                        <i class="bi bi-link-45deg"></i>{{ $r?->no_reimburse }}
                    </a>
                </div>
            </div>
        </div>
